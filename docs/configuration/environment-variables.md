...
| `FLOCI_SERVICES_CODEBUILD_CURATED_IMAGE_SUBSTITUTE` | `public.ecr.aws/codebuild/amazonlinux-x86_64-standard:5.0` | Image run in place of curated `aws/codebuild/*` names that AWS does not publish publicly (the Ubuntu `standard` family); Amazon Linux curated names map to their public.ecr.aws mirrors directly |
