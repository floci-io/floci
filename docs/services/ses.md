...
`Reject` is emitted the way AWS documents it: a message carrying the [EICAR test file](https://www.eicar.org/download-anti-malware-testfile/) in its subject, a header or a text body is accepted (the send returns a `MessageId`) and then rejected with a `Reject` event whose `reason` is `Bad content`; the message is not relayed. The test string itself is deliberately not reproduced here.
