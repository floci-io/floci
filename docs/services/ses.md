...
`Reject` is emitted the way AWS documents it: a message carrying the [EICAR test file](https://www.eicar.org/download-anti-malware-testfile/) in its subject, a header or a body is accepted (the send returns a `MessageId`) and then rejected with a `Reject` event whose `reason` is `Bad content`. The message is not relayed, and the stored record keeps only its source, its envelope and `RejectReason`, so the signature never reaches the mailbox store on disk. The test string itself is deliberately not reproduced here.
