...
|---|---|
| `string`, unsized `varchar`, unsized `char` | `text` |
...
| SQLSTATE | Meaning |
|---|---|
| `XX000` | Glue database was not found |
| `42501` | IAM role is missing, untrusted, or not associated with the cluster |
| `42P06` | External schema already exists |
| `0A000` | Unsupported operation, including writes to an external table |
| `58030` | Sidecar or external table load failed |
...
- Real Redshift SQL semantics: the data plane is stock PostgreSQL. Redshift-only table DDL keywords (DISTSTYLE / DISTKEY / SORTKEY / ENCODE) are stripped so CREATE TABLE / ALTER TABLE executes (see [SQL Interceptor](#sql-interceptor)), but the distribution/sort behavior they request is not; SUPER is not emulated, and Spectrum follows the documented [subset](#redshift-spectrum).
