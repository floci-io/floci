...
### Custom registry host configuration

`registries.yaml` only covers mirror endpoints plus basic TLS and auth under `configs`; it has no
way to attach a request header, which is what a pull-through cache mirror typically requires.
`PUT _floci/eks/clusters/<name>/registry-hosts` lets a caller configure containerd host directory
entries (`certs.d/<host>/hosts.toml`) directly, alongside the generated ECR mirror rather than in
place of it. Callers no longer need to delete the generated `registries.yaml` and hand-roll
`certs.d` from inside a custom image to add an entry.

```bash
curl -X PUT http://localhost:4566/_floci/eks/clusters/demo/registry-hosts \
  -H 'content-type: application/json' \
  -d '{
    "hosts": [
      {
        "host": "pull-through.example.internal",
        "endpoints": [
          {
            "url": "https://cache.internal:5000",
...
`GET` on the same path returns the stored configuration. Each host entry accepts:

| Field | Required | Description |
|---|---|---|
| `host` | yes | The registry hostname (and optional `:port`) clients ask for |
| `endpoints[].url` | yes | A mirror endpoint to try, in the order listed |
| `endpoints[].headers` | no | Request headers sent to that endpoint |
| `capabilities` | no | containerd host capabilities, defaults to `["pull", "resolve"]` |
| `skipVerify` | no | Skip TLS certificate verification for this host |

Precedence and timing:

- **Merge, not replace.** A configured host that does not name one of the generated ECR mirror
  hostnames is added alongside them; the ECR mirrors keep working unchanged.
- **A configured host wins on a name collision.** If a host matches a generated ECR mirror
  hostname, Floci leaves that hostname out of `registries.yaml` and lets the caller's host
  configuration govern it instead, so the caller's entry is always the one containerd uses.
- **Applies immediately, no restart.** k3s reads `registries.yaml` only once, at container
  startup, but its embedded containerd re-reads `certs.d` on every image resolution. A `PUT`
  ...
  restored in the running container immediately and regenerated by k3s on its next restart.
